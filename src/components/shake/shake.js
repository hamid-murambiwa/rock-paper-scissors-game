...
function Shake() {
  ...
  const triggerFade = (opR, opL) => {
    ...
    setTimeout(() => { setOptionRight(opR); }, 1200);
    setTimeout(() => { setOptionLeft(opL); }, 1200);
    ...
    <div className="container">
      <section className="scores">
        ...
        <div className="sub-con">
          <div className="sub-reset">
            <p>Reset scores</p>
            <button type="button" className="res-btn" onClick={() => { resetF(); }}><img id={reset ? 'reset-ani' : 'reset'} src={resetButton} alt="reset button icon" /></button>
          </div>
      ...
      <div className="bottom-con">
        ...
        <section className="a-con">
          <button type="button" className="res-btn" id="p-btn" onClick={() => { setButtons(); }}>Play Again</button>
          <div className="t-a">
            <button type="button" className="res-btn" onClick={() => { game(1); }}><img id={rockAni ? 'rock-ani' : 'rock'} src={rockButton} alt="rock icon" /></button>
          </div>
          <div className="t-b">
            <button type="button" className="res-btn" onClick={() => { game(2); }}><img id={paperAni ? 'paper-ani' : 'paper'} src={paperButton} alt="paper icon" /></button>
            <button type="button" className="res-btn" onClick={() => { game(3); }}><img id={scissorsAni ? 'scissors-ani' : 'scissors'} src={scissorsButton} alt="scissors icon" /></button>
          </div>
        </section>
      </div>
      <button type="button" className="res-btn" id="begin" onClick={() => { begin(); }}>Let&apos;s play</button>
    </div>
  );
}
